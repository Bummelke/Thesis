...
to setup
  clear-all
  ask patches [set pcolor one-of [brown green]
    ifelse pcolor = green
    [set countdown grass-countdown]
    [set countdown random grass-countdown]
  ]

  repeat num-fragments[
    draw-roads num-fragments]
...
to draw-roads [fragments]
    let random-y one-of n-values (max-pycor - min-pycor + 1) [ i -> i + min-pycor ]

    ask patches with [
      pycor >= (random-y -  (1.0 / fragment-size) * 100) and
      pycor <= (random-y +  (1.0 / fragment-size) * 100)
    ] [
      set pcolor gray
    ]

      let random-x one-of n-values (max-pycor - min-pycor + 1) [ i -> i + min-pycor ]

    ask patches with [
      pxcor >= (random-x -  (1.0 / fragment-size) * 100) and
      pxcor <= (random-x +  (1.0 / fragment-size) * 100)
    ] [
      set pcolor gray
    ]
...
"" ""
PENS
"foxes" 1.0 0 -3844592 true "" "plot count foxes"
"rabbits" 1.0 0 -6759204 true "" "plot count rabbits"
"grass" 1.0 0 -13840069 true "" "plot count count-grass / 4"
...
@#$#@#$#@
<experiments>
  <experiment name="Pop dynamics scriptie fragment" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    ...
    <enumeratedValueSet variable="num-fragments">
      <value value="2"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="fragment-size">
      <value value="40"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="Pop dynamics scriptie classic" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count foxes</metric>
    <metric>count rabbits</metric>
  ...
  <experiment name="Pop dynamics scriptie fragment centroid" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>count foxes</metric>
    <metric>count rabbits</metric>
